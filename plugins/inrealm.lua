...
local function get_group_type(msg)
  ...
  if data[tostring(msg.to.id)] then
    if not data[tostring(msg.to.id)]['group_type'] then
     return 'هیچ نوع گروهی وچود ندارد.'
    end
     local group_type = data[tostring(msg.to.id)]['group_type']
     return group_type
  else 
     return 'نوع گروه یافت نشد.'
  end 
end
...
local function lock_group_flood(msg, data, target)
    ...
    local group_flood_lock = data[tostring(target)]['settings']['flood']
        if group_flood_lock == 'yes' then
            return 'فلوود گروه قفل است'
        else
            data[tostring(target)]['settings']['flood'] = 'yes'
            save_data(_config.moderation.data, data)
        return 'فلوود گروه قفل شد'
        end
end
...
local function admin_promote(msg, admin_id)
        ...
        if data[tostring(admins)][tostring(admin_id)] then
                return admin_name..' از قبل ادمین بوده است.'
        end
        data[tostring(admins)][tostring(admin_id)] = admin_id
        save_data(_config.moderation.data, data)
        return admin_id..' به عنوان ادمین انتخاب شد.'
end
...
local function admin_list(msg)
    local data = load_data(_config.moderation.data)
        ...
        local message = 'لیست ادمین هایه روبات:\n'
...
local function admin_user_promote(receiver, member_username, member_id)
        ...
        if data['admins'][tostring(member_id)] then
                return send_large_msg(receiver, member_username..' از قبل ادمین بوده است.')
        end
        data['admins'][tostring(member_id)] = member_username
        save_data(_config.moderation.data, data)
        return send_large_msg(receiver, '@'..member_username..' به عنوان ادمین انتخاب شد.')
end
...
local function admin_user_demote(receiver, member_username, member_id)
    local data = load_data(_config.moderation.data)
        ...
        save_data(_config.moderation.data, data)
        return send_large_msg(receiver, 'ادمین '..member_username..' از برکنار شد.')
end
...
local function username_id(cb_extra, success, result)
   ...
   local text = 'کاربری با نام @'..member..' در این گروه وجود ندارد.'
   for k,v in pairs(result.members) do
...
local function set_log_group(msg)
  ...
  if log_group == 'yes' then
    return 'آمار گروه از قبل تنظیم شده است'
  else
    data[tostring(groups)][tostring(msg.to.id)]['log_group'] = 'yes'
    save_data(_config.moderation.data, data)
    return 'آمار گروه تنظیم شد'
  end
end
...
local function unset_log_group(msg)
  ...
  if log_group == 'no' then
    return 'آمار گروه غیرفعال است'
  else
    data[tostring(groups)][tostring(msg.to.id)]['log_group'] = 'no'
    save_data(_config.moderation.data, data)
    return 'آمار گروه غیرفعال شد'
  end
end
